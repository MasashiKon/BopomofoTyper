...
const score = ref(0)
const rank = ref(0)
const level = ref(/*nuxtStorage.localStorage.getItem(LocalStrageName.level) */ null || Level.easy)
const username = ref('')
const volume = ref(/*nuxtStorage.localStorage.getItem(LocalStrageName.volume)*/ null || '50')
const isVolumeOn = ref(
  /*nuxtStorage.localStorage.getItem(LocalStrageName.isVolumeOn) === 'true'*/ Math.random() > 0.5
    ? true
    : false || false
)
const verticalZhuyin = ref(
  /*nuxtStorage.localStorage.getItem(LocalStrageName.verticalZhuyin) === 'true'*/ Math.random() >
    0.5
    ? true
    : false || false
)
const hideZhuyin = ref(
  /* nuxtStorage.localStorage.getItem(LocalStrageName.hideZhuyin) === 'true'*/ Math.random() > 0.5
    ? true
    : false || false
)
...
const setVolume = () => {
  ...
  // nuxtStorage.localStorage.setItem(LocalStrageName.volume, volume.value)
}
...
onMounted(async () => {
  ...
  supabase = createClient(
    process.env.VITE_DB_URL_GEN as string,
    process.env.VITE_DB_APIKEY as string
  )
    ...
    if (e.key === 'ArrowUp') {
      switch (level.value) {
        case Level.practice:
          ...
          // nuxtStorage.localStorage.setItem(LocalStrageName.level, Level.hard)
          break
        case Level.easy:
          level.value = Level.practice
          // nuxtStorage.localStorage.setItem(LocalStrageName.level, Level.practice)
          break
        case Level.hard:
          level.value = Level.easy
          // nuxtStorage.localStorage.setItem(LocalStrageName.level, Level.easy)
          break
      }
...
const changeLanguage = (lang: string) => {
  ...
  // nuxtStorage.localStorage.setItem(LocalStrageName.userLang, lang)
}
...
const toggleVolume = () => {
  ...
  // nuxtStorage.localStorage.setItem(LocalStrageName.isVolumeOn, String(isVolumeOn.value))
}

const toggleVerticalZhuyin = () => {
  verticalZhuyin.value = !verticalZhuyin.value
  // nuxtStorage.localStorage.setItem(LocalStrageName.verticalZhuyin, String(verticalZhuyin.value))
}

const toggleHideZhuyin = () => {
  hideZhuyin.value = !hideZhuyin.value
  // nuxtStorage.localStorage.setItem(LocalStrageName.hideZhuyin, String(hideZhuyin.value))
}
