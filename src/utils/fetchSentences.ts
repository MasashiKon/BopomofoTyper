...
const generateSentences = (
  ...
  const resSentences: any[] = res.data.data[`sentences_easy_${innerLevel}Collection`].edges
  ...
  if (level === Level.easy) {
    for (const sentenceNode of randomizedSentences) {
      ...
      for (const chunkLiteral of sentence.chunks) {
        chunkSearch: for (const chunkNode of sentence[`easy_${innerLevel}_chunksCollection`]
          .edges) {
        ...
        chunkSearch: for (const wordNode of sentence[`easy_${innerLevel}_wordsCollection`].edges) {
          const word = wordNode.node.words
...
export default async (sentences: SentenceContainer, level: Level) => {
  ...
  const res2 = await axios.post(
    import.meta.env.VITE_DB_URL,
    {
      query: generateQuery(level, innerLevel)
    },
    {
      headers: {
        apikey: import.meta.env.VITE_DB_APIKEY
  ...
  generateSentences(res2, sentences.high, innerLevel, level, translationIndex)
}
